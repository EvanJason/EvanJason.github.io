...
## 16.什么是BFC（Block Formatting Context）
...
## 21.在 ES6 中，let 声明的变量可以修改，而 const 声明的变量不可更改，使用 const 声明必须指定初始值。
...
需要明确的是，`const` 保证的是**变量指向的内存地址不变**，而不是值不可变。对于引用类型，虽然不能重新赋值，但可以修改其内部属性或元素。
...
// 注意：freeze 是浅冻结，嵌套对象仍可修改
const nested = Object.freeze({ 
  user: { name: 'Alice' } 
});
nested.user.name = 'Bob'; // 成功！
...
deepFrozen.user.name = 'Bob'; // 静默失败
console.log(deepFrozen.user.name); // 'Alice'
```
...
| 数据类型 | 能否重新赋值 | 能否修改内容 |
|---------|------------|------------|
| 基本类型（Number、String等） | ❌ 否 | - |
| 引用类型（Object、Array） | ❌ 否 | ✅ 是 |
| 冻结的对象 | ❌ 否 | ❌ 否（浅冻结） |
| 深层冻结的对象 | ❌ 否 | ❌ 否 |

**核心要点：**
- `const` 保证的是**引用的不可变性**，不是值的不可变性
- 对于基本类型，值本身就存储在变量中，所以看起来不可修改
- 对于引用类型，变量存储的是内存地址，地址不变但内容可变
- 如需完全不可变，使用 `Object.freeze()` 或 Immutable.js 等库
...
## 40.前端SPA（Single Page Application）的原理
...
## 51.React 状态管理：Redux、MobX、Zustand 对比
...
Vue 2.4+ 引入了**双端比较**优化，从前后两端同时向中间比较:
...
|------|-------|-------|-------|
| **时间复杂度** | O(n) | O(n) | O(n) |
| **比较方式** | 单端循环 | 双端比较 | 双端 + LIS |
| **Key 要求** | 必须 | 推荐 | 推荐 |
| **节点移动** | 依赖 key | 双端优化 | LIS 最优解 |
| **静态优化** | useMemo 手动 | 自动识别 | 编译时标记 |
| **跳过 Diff** | React.memo | 自动 | Patch Flags |
| **更新粒度** | 组件级 | 节点级 | 属性级 |
